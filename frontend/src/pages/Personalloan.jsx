...
const Personalloan = () => {
    ...
    const getHeaders = () => {
        return {
            ...
            authorization: `Bearer ${localStorage.getItem("token")}`, // Corrected syntax for template string
        };
    };

    // Function to fetch personal loans
    const fetchLoans = async () => {
        try {
            const headers = getHeaders();
            const response = await axios.get("http://localhost:1000/api/v2/get-personal-loans", { headers });
            setPersonalLoans(response.data.personalLoans || []); 
        } catch (error) {
            console.error("Error fetching personal loans:", error.response ? error.response.data : error.message);
        }
    };
    ...
    const handleLoanSubmit = async (loan) => {
        ...
        try {
            ...
            setPersonalLoans(prevLoans => [...prevLoans, newLoan]); // Update state with new loan
            
            // Reload the Personalloan page
            navigate('/personalloan'); // Navigate to the Personalloan page
    ...
    const handleLoanDelete = async (loanId) => {
        ...
        try {
            ...
            setPersonalLoans(prevLoans => prevLoans.filter(loan => loan._id !== loanId));
        } catch (error) {
        ...
        <div>
            <div className="loan-container scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                {personalLoans.length > 0 ? (
                    ...
                    <div className="text-center text-gray-500">No personal loans available.</div>
                )}
            </div>
