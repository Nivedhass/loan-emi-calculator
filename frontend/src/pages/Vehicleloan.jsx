...
import LoanCards from '../components/Home/LoanCards'; // Reuse LoanCards for displaying loans
import UpdateLoan from '../components/Home/UpdateLoan'; // Reuse UpdateLoan for updating loans
import InputLoan from '../components/Home/InputLoan'; // Import InputLoan for adding new loans
import axios from 'axios';

const VehicleLoan = () => {
    const navigate = useNavigate(); 
    const [inputDiv, setInputDiv] = useState("hidden"); // State for input form visibility
    ...
    // Function to fetch vehicle loans
    const fetchLoans = async () => {
        try {
            const headers = getHeaders();
            const response = await axios.get("http://localhost:1000/api/v2/get-vehicle-loans", { headers });
            setVehicleLoans(response.data.vehicleLoans || []); 
        } catch (error) {
            console.error("Error fetching vehicle loans:", error.response ? error.response.data : error.message);
        }
    };
    ...
    const handleLoanSubmit = async (loan) => {
        ...
        try {
            ...
            setVehicleLoans(prevLoans => [...prevLoans, newLoan]); // Update state with new loan
            navigate('/VehicleLoan'); // Navigate to the VehicleLoan page
    ...
    const handleLoanDelete = async (loanId) => {
        ...
        try {
            ...
            setVehicleLoans(prevLoans => prevLoans.filter(loan => loan._id !== loanId));
        } catch (error) {
        ...
        <div>
            <div className="loan-container scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                {vehicleLoans.length > 0 ? (
                    ...
                    <div className="text-center text-gray-500">No vehicle loans available.</div>
                )}
            </div>
